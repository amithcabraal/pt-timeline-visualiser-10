...
export function EventTable({ onEditEvent, events, timelineStart, timelineEnd }: EventTableProps) {
    ...
    <div className="bg-white rounded-lg shadow overflow-hidden">
      <div className="p-4 bg-gray-50 border-b">
        <h2 className="text-lg font-semibold text-gray-700">
          Events from {format(timelineStart, 'MMM d, yyyy')} to {format(timelineEnd, 'MMM d, yyyy')}
        </h2>
      </div>
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {events.map((event) => (
              <tr key={event.id} onDoubleClick={() => onEditEvent(event)} className="hover:bg-gray-50 cursor-pointer">
